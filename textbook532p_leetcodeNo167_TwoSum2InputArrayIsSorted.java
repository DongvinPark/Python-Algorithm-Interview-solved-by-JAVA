...
class Solution {
    ...
    public int[] twoSum(int[] numbers, int target) {
        ...
        //입력 배열을 차례대로 순회하되, 현재 i번째 요소를 보고 있다면, i+1 ~ numbers.length-1 인덱스까지의 범위에 대해 이진탐색을 실시합니다.
        //탐색 결과가 문제의 조건과 일치한다면 정답에 포함시킵니다. 문제에서 정답은 유일하다 했으므로 정답을 찾았다면 나머지 요소들은 더 이상 볼 필요가 없습니다. 따라서 break; 를 활용해서 실행시간을 단축시킬 수 있습니다.
...
class Solution {
    public int[] twoSum(int[] numbers, int target) {
        ...
        while(left < right) {
            ...
            //합이 타깃보다 작다면 왼쪽 포인터를 ++하여 합이 커질 수 있게 해줍니다. 이게 가능한 이유는 입력 배열이 오름차순으로 정렬되 있는 배열이기 때문입니다.
            else if(sum < target) {
                left++;
            }

            //합이 타깃보다 크다면 오른쪽 포인터를 --하여 합이 줄어들게 해줍니다. 이게 가능한 이유는 입력 배열이 오름차순으로 정렬돼 있는 배열이기 때문입니다.
            else if (sum > target) {
                right--;
            }
