...
// 직렬화의 결과물로서 스트링을 반환할 것을 제시하고 있는데, 스트링에서 null인 노드를 어떻게 처리할지는 문제를 푸는 사람의 마음대로 하면 됩니다. 코드 맨 아래 부분의 주석에서 그것에 대한 내용을 주석으로 알려주고 있습니다.
//이번 풀이는 while 문과 LinkedList 큐를 이용한 BFS 풀이로 접근했고, 실행시간은 9밀리초에 상위 약 9%의 성능이었습니다.
//트리를 직렬화 할때, 노드가 널이 아닌 경우엔 숫자와 공백 하나를 스트링빌더에 추가하고, 노드가 널인 경우엔 N이라는 문자와 공백 하나를 스트링빌더에 추가해줍니다.
public class Codec {
    ...
    public String serialize(TreeNode root) {
        ...
        while(list.isEmpty()==false){
            ...
            //이때 자식 노드가 널인 경우에도 일단 리스트에 넣을 수는 있습니다. 이렇게 되었다 해도, 어차피 다음 while루프에서 else 파트가 null 부분을 처리해주기 때문에서 굳이 여기에서 null 처리를 따로 해줄 필요가 없습니다.
            if(node != null){
    ...
    public TreeNode deserialize(String data) {
        ...
        이진 트리가 
                   ...
                   2       3
                4   5     6   7
        이렇게 생긴 상황에서 이것을 직렬화 했을 때, 1번 노드는 1 인덱스에 넣으면 되고, 2번 노드는 2 인덱스에, 3번 노드는 3 인덱스로 가면 됩니다.
        ...
        */
        ...
        while(list.isEmpty()==false){
            ...
            //여기서 인덱스를 +1 시킨 것이 중요합니다. 인덱스를 +1 해줌으로써 현재 노드인 node의 왼쪽 자식 노드에 이어서 바로 오른쪽 자식 노드를 treeArray[]에서 접근할 수 있기 때문입니다.
            index +=1;
