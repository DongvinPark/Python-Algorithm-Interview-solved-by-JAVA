...
class Solution {
    ...
    //무려 5개의 인자를 전달해야 했습니다. String[] keyB는 >>> P <<<의 위치에 private static final 변수로 선언할 경우 메서드의 인자를 5개에서 4개로 줄일 수 있습니다.
    public void search(int i, StringBuilder sb, String digits, List<String> answer, String[] keyB ){
        // 첫 파라미터인 i는 인트 타입이고, 세번째 파라미터인 digits는 String입니다. 입력으로 전달된 digits를 int 타입 숫자로 바꿔서 String[] keyB에서 탐색 대상 문자열을 골라낸 후, 다음 재귀호출을 실시할 때 i+1을 다음 재귀호출의 인자로 전달해야 하기 때문입니다.
        ...
        //digits.charAt(i)-'0' 의 의미를 이해하는 것이 필요합니다. int a = '1';을 출력해보면, 49이고, int b = '0';을 출력해보면 48입니다. 따라서 a-b == 1이 되고, 이는 '1'이라는 문자가 컴퓨터 공학적인 의미의 엄밀한 의미가 아니라 겉으로 보이는 값이 1과 일치합니다. 따라서 digits.charAt(i)-'0'는 사실 digits의 첫 번재 글자를 정수로 바꿔주는 것과 같습니다.
...
a--------------------------d >>> a와 d를 sb에 찾았으므로, 이 sb를 List<Stirng> answer에 추가해야 합니다.
                           |
                           e >>> 이번 호출에서는 sb가 a와 e를 갖고 있어야 합니다. 그런데, 여기서 직전 호출의 sb인 ad에 그대로 e를 추가할 경우 오답이 됩니다. d를 제거한 후, b를 더해야 정답입니다. 나머지 다른 호출들에 대해서도 마찬가지 압니다.
                           |
                           f
