...
class Solution {
    public int networkDelayTime(int[][] times, int n, int k) {
        ...
        //아래의 while루프를 시작할 수 있어야 하므로 큐에 시작점 k와 도착시간 0으로 구성된 pair를 하나 넣어 줍니다.
        //출발점 k에서 시작하여 도착점이 자기 자신이므로 시간은 당연히 0입니다.
        ...
        while (!que.isEmpty()) {
            ...
            if(!visited.contains(node)){
                ...
                for (int[] nextNodes: graph.get(node)) {
                	//만약 nextNodes 배열의 요소가 이미 방문한 지점이 아니라면,
                    //'해당 노드'와 '해당 노드가 신호를 받는데 걸리는 시간'의 쌍을 큐에 집어넣어야 합니다.
