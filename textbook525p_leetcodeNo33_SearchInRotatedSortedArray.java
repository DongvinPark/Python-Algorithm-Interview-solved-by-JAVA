...
//1,2 밀리초로 측정된 풀이들에 비해서 좀 더 직관적으로 이해할 수 있지만, 인덱스를 직접 다루기 때문에 관련 경우의 수를 나눌 때 세심히 주의를 기울여야 한다는 단점도 있습니다.
/*
...
* 여기서 pivotIdx, 즉, 배열의 회전이 일어난 인덱스 값은 원래 입력 배열인 nums[] 배열에서 최솟값이 위치하고 있는 인덱스 값과 동일하다는 점을 알 수 있습니다.
* 13579배열이 pivotIdx == 3 값을 기준으로 회전한 것이 원래 입력배열인 nums[]인 것입니다.
* 일단 타깃 값을 13579 배열에서 이진 탐색을 이용해서 찾습니다. 그 값으로 answer를 초기화 해줍니다.
*
* nums[nums.length-1] 즉 원래 입력 배열의 마지막에 위치한 값을 기준으로, 이 값(코드에서는 oriPvValue 입니다)보다 작거나 같은 값인 1,3은 13579배열에서 57913 배열로 회전할 때, 13579배열의 0,1인덱스에서 pivotIdx(==3)만큼 더해져야 정답이 될 수 있음을 알 수 있습니다. 이 경우를 A 케이스라고 하겠습니다.
*
* oriPvValue 값보다 큰 값들인 5,7,9는 pivotIdx 값을 기준으로 배열을 회전시켰을 때 13579배열의 2,3,4인덱스에서 (nums.length-pivotIdx) 값만큼 빼줘야 정답이 됨을 알 수 있습니다. 이 경우를 B 케이스라고 하겠습니다.
*
* */
class Solution {
...
//두 번째 풀이는 첫 번째 풀이보다 더 직관적입니다. 두 번째 풀이이 핵심은 회전된 배열이라 할지라도, 피벗 인덱스 값을 기준으로 배열의 두 부분을 나누면 각각의 배열은 어차피 정렬이 다 돼 있는 상태이기 때문에 nums[] 배열을 두 개로 나눠서 각각에 대해 이진탐색을 해준다는 것입니다.
/*
