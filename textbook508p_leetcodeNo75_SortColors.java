//이 문제는 네덜란드 국기 문제로 잘 알려진 문제입니다. 정렬 라이브러리를 쓰면 안 되고,
//공간 복잡도를 O(n)으로 만드는 풀이 또한 불가능합니다.
//공간복잡도를 O(n)으로 만드는 풀이를 허용할 경우, PIVOT인 1을 기준으로 더 작은 값인 0이 등장하는 횟수와
//1보다 큰 2가 등장하는 횟수, 그리고 1이 등장하는 횟수를 기록해서 단순히 0,1,2의 개수의 합만큼의 공간을 할당한 배열을 만들어서
//각자의 등장 횟수에 맞게 0,1,2를 채워 넣으면 끝나기 때문입니다.
//이 문제풀이에 대한 설명은 네덜란드 국기 문제에 대한 위키백과의 설명에서 자세히 나와 있으므로 자세한 설명은 하지 않으려하는데,
//저는 이 코드가 실제로 어떻게 작동하는지를 추적하는 로그를 만들어봤습니다. 풀이를 이해하는데 도움이 되리라 생각됩니다.
