...
* 첫 루프에서는 더미헤드 노드의 다음 노드는 null일 수밖에 없기 때문에, [ 더미노드를 가리키고 있는 curNode ]의 다음 노드도
* 당연히 null일 수밖에 없습니다.
* 그래서 로그에서 ListNode next is NULL 라는 문장이 출력된 것입니다.
* 새로운 노드를 만들어서 이어 붙이기 전에, curNode.next를 가리키고 있는 새로운 노드은 next 노드를 만들어줍니다.
* 이때, curNode는 여전히 더미헤드 노드를 가리키고 있으며, curNode.next 는 null이라는 것을 기억해야 합니다.
* null 상태인 curNode.next 노드에 new 연산을 사용해서 새로운 노드를 만들어줍니다.
* 그후, curNode.next의 다음 노드를 next로 설정해 줍니다.
...
* 첫 루프와 마찬가지로 더미헤드를 가리키는 curNode를 새로 만들어줍니다. 이때, curNode.next의 값은 4입니다.
* 왜냐하면, curNode가 dummyHead노드를 가리키고 있기 때문입니다. head 노드의 값이 3인데 이는 4보다 작기 때문에 안쪽 루프는 진입을 할 필요가 없습니다.
*
* 여기서부터 next 노드의 진짜 역할이 드러나기 시작합니다. next 노드는 4노드를 가리키기 위해서 존재합니다.
* 4 노드는 2 노드보다 정렬 순서 상 나중에 등장해야 하는 노드입니다.
* 그런데 더미헤드 노드 다음에 2노드가 위치하도록 만들어놓을 경우, 더미헤드 노드 바로 다음에 위치하게 만든 2노드와 4노드를
* 서로 연결시켜 줄 때 사용할 수 있는 참조 값이 사라지게 됩니다.
* 정답을 만들어내려면 당연히 2노드 다음에 4노드가 오도록 연결시켜줘야 하는데, 더미헤드 노드 다음에 2노드를 새로 만들어서 붙여주는 과정에서
* 더미헤드와 4노드 사이의 연결이 끊어져버리는 것입니다.
...
*
*
* 네 번째 루프에서는 마침내 안쪽 루프에 실제로 진입하게 됩니다. 현재 head는 3 값을 가리키고 있는데,
* 이것은 1,2 보다 나중에 등장해야 하기 때문입니다.
* 바깥 루프에 들어서면 그동안 해왔던 것처럼 더미헤드 노드를 가리키는 curNode를 새로 만들어줍니다.
* 이때, curNode.next의 값이 현재 head노드가 가리키는 값보다 작기 때문에 curNode가 가리키는 위치를 수정해야 할 필요성이 있습니다.
* curNode가 가리키는 위치를 수정하기 위한 로직을 맡는 부분이 안쪽 while루프 입니다.
*
* 안쪽 루프에서 비교할 때, curNode.next의 값과 head의 값을 서로 비교하는 것에 유의해야 합니다.
* 새로운 노드를 만들어주는 위치가 curNode가 아니라 curNode.next이기 때문에,
* curNode.next의 값보다 작을 때까지 curNode를 이동시켜야 하기 때문입니다.
