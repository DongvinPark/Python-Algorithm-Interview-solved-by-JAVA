...
//실행시간은 1밀리초였고, 이는 상위 약 50%의 성능이었습니다.
    ...
    //문제에서 말하는 '거리'를 구하기 위해서 노드 간의 거리 횟수를 구할 때 필연적으로 중간 노드들이 존재할 수밖에 없습니다. 이때 그 중간노드들 중 하나는 반드시 루트 노드여야합니다. '최대 거리'는 루트 노드를 경유하지 않고서는 도출될 수 없기 때문입니다.
    //이번 문제풀이에서는 '상태값'의 의미를 이해하는 것이 중요합니다. 특정 노드의 상태값이란, 그 노드를 루트노드로 가정했을 때 그 노드에서 리프 노드까지 도달하는데 이동해야 하는 횟수 중 최댓값을 뜻합니다.
    //이 문제의 정답은 반드시 루트노드를 포함하는 경로를 통해서만 얻을 수 있으므로, 최종 정답은 문제에서 주어진 루트 노드의 왼쪽 차일드 노드의 상태값과 오른쪽 차일드 노드의 상태값의 합임을 알 수 있습니다.
class Solution {
    ...
    public static int dfs(
        ...
        //dfs 호출이 null을 만날 때까지 이어진 후에서야 dfs 메서드의 return Math.max(left,right) + 1; 부분이 순차적으로 호출되는 구조를 선택한 것에 주목해야 합니다.
        //이러한 탐색을 왼쪽 차일드 노드와 오른쪽 차일드 노드에 모두 해준 다음 기존 longest 값과 (leftStatusValue + rightStatusValue) 값 중 큰 값으로 longest 값을 갱신해줍니다.
