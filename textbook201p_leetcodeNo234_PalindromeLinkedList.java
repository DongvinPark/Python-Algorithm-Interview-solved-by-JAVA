...
//슬로우 포인터는 한 칸씩 이동하고, 페스트 포인터는 2칸씩 이동합니다. 결과적으로 페스트 포인터가 리스트의 끝에 도착한 후 한 번 더 앞으로 가면, 슬로우 포인터는 입력 리스트이 요소 개수가 홀수인 경우 중앙의 요소 바로 다음 요소에 위치하게 되고, 입력 리스트의 요소 개수가 짝수인 경우, 예를 들어서 리스트가 1,2,3,3,2,1일 때, 두 번째 등장한 3의 위치에 위치하게 됩니다.
/**
 * Definition for singly-linked list.
 * public class ListNode {
 *     int val;
...
class Solution {
    public boolean isPalindrome(ListNode head)
    {
        ...
        ListNode reversed = reverse(slow);//reverse라는 메서드가 ListNode 타입의 노드를 반환한다는 것을 아는 것이 중요합니다. 해당 메서드에서 반환된 노드는 1,2,3,3,2,1이 전체 입력 노드일 때, 오른쪽 절반에 해당하는 3,2,1을 역순으로 뒤집어서 만든 1,2,3 리스트의 첫 시작 노드를 반환하기 때문입니다. 따라서 원본 입력 리스트가 1,2,3,3,2,1 이었다면, reversed 노드와 head 노드는 결과적으로 동일한 요소를 포함하는 리스트들이 됩니다.
